...
# ----------------------------------------------------------------------------------------
    ...
    changeSensorText= Signal(int)
    changeSensorValue = Signal(float)
    ...
    def getConnect(self):
        ...
        if self.isButtonClicked is True:
            ...
            self.changeConnectionButtonColor.emit("#00b156") # Button Color
            ...
            # Sensor
            sensor = vehicle.rngfnd
            sensor_int = abs(float(sensor))
            self.changeSensorText.emit(sensor_int)
            print(f"\naltitude [gelen] = {sensor}")
            sensor = mapFromTo(abs(sensor), 0, 40, 0, 360)

            self.changeSensorValue.emit(sensor)
            print(f"\naltitude [giden] = {sensor}")
